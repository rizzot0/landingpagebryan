...
function BlogBlock({ block }) {
  ...
  if (block.type === 'ctaWhatsApp') {
    return (
      <div className="pt-2">
        <a
          href={wsp(block.message || 'Hola Brayan, quiero conversar sobre mi proyecto.')}
          target="_blank"
          rel="noopener noreferrer"
          className="inline-flex w-full items-center justify-center rounded-xl bg-green-500 px-6 py-3.5 text-sm font-semibold text-white transition-colors hover:bg-green-600 sm:w-auto"
        >
          {block.label || 'Enviar mensaje por WhatsApp'}
        </a>
      </div>
    )
  }
